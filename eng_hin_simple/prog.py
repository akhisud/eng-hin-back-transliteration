...
class keras_model:
	def __init__(self,dataset):
		...
		self.eng_ind_to_char = dataset.eng_ind_to_char
		self.model = None
	...
	def config(self):
		...
		eng_inp = Input(shape=(self.e_max,),dtype='float32')
		print eng_inp
		eng_emb = Embedding(len(self.eng_ind_to_char),emb_dim)(eng_inp)
		print eng_emb
		enc_forward = LSTM(enc_dim, return_sequences=False)(eng_emb)
		print enc_forward
		enc_backward = LSTM(enc_dim, return_sequences=False)(eng_emb)
		print enc_backward
		encoder = concatenate([enc_forward,enc_backward])
		print encoder
		decoder_inp = concatenate([eng_emb,RepeatVector(self.e_max)(encoder)])
		decoder = LSTM(dec_dim, return_sequences=True)(decoder_inp)
		dense = Dense(self.h_max,activation='softmax')(encoder)
		self.model = Model(input= eng_inp, output= dense)
